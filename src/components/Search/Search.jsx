...
class Search extends Component {
  ...
  onClickedSend = () => {
    ...
    const checkValue = dataList.filter(
      (obj) => obj.q.toLowerCase() === inputValue.toLowerCase()
    );
      ...
      <div>
        <div className={styles.searchMainCont}>
          <input
            type="text"
            value={inputValue}
            onChange={this.onEnteredInputValue}
        ...
        {inputStatus === "empty" && (
          <p className={styles.errorMsg}>Please Provide non-empty Input</p>
        )}
        {inputStatus === "unrecognized" && (
          <p className={styles.errorMsg}>Please Provide Valid Input</p>
        )}
      </div>
    );
  }
